...
\newcommand{\guideDesignation}{Assistant Professor, Department of CSE}
...
\begin{itemize}
    \item Lack of reliable, accurate, and real-time market information.
    \item Dependence on intermediaries for price insights.
    \item Difficulty identifying the best time to sell to maximize profits.
    \item Limited knowledge about market trends and influencing factors.
\end{itemize}

\subsection{For Merchants}
\begin{itemize}
    \item Difficulty tracking regional price variations.
    \item Inability to analyze historical and predicted price movements.
    \item Challenges in planning procurement and distribution efficiently.
\end{itemize}

\subsection{For Consumers}
\begin{itemize}
    \item Fluctuating retail prices with little transparency.
    \item Lack of understanding about seasonal availability and price cycles.
\end{itemize}
...
\begin{itemize}
    \item Limited access to live agricultural price APIs
    \item Highly unpredictable crop price behavior
    \item Free-tier API limits (weather, news, chatbot)
    \item Performance constraints in rendering large datasets
\end{itemize}
...
\begin{itemize}
    \item Real-time and historical price data
    \item Predictive analytics
    \item Interactive visualizations
    \item API-based integrations
    \item Responsive UI/UX
    \item AI chatbot interactions
\end{itemize}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Three-tier system architecture diagram placeholder]}\vspace{2cm}}}
\caption{Three-tier system architecture of AgroVision showing the Presentation Layer (Frontend UI), Application Layer (Backend/API), and Data Layer (Database \& External Integrations)}
\label{fig:three_tier_arch}
\end{figure}

\subsection{Architectural Layers}

\subsubsection{Presentation Layer (Frontend UI)}
\begin{itemize}
    \item Developed using React JS + Tailwind CSS
    \item Represents the user interface visible to farmers, merchants, and customers
    \item Handles charts, dashboards, role-based views, chatbot UI buttons
    \item Communicates with backend via REST API calls
\end{itemize}

\subsubsection{Application Layer (Backend / API Layer)}
Implemented using Node.js (Express) or Python (FastAPI/Flask). Responsible for:
\begin{itemize}
    \item Serving REST APIs
    \item Processing price history queries
    \item Executing AI prediction logic
    \item Fetching news \& weather data
    \item Integrating chatbot responses
    \item Managing business logic for different user roles
\end{itemize}

\subsubsection{Data Layer (Database + External Integrations)}
Stores:
\begin{itemize}
    \item Crop details
    \item Price history
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[High-level architecture diagram placeholder]}\vspace{2cm}}}
\caption{High-level architecture diagram illustrating the interaction between User Interface, Backend API Services, Database, and External API integrations (Weather, News, and Chatbot services)}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Use case diagram placeholder]}\vspace{2cm}}}
\caption{Use case diagram depicting the interactions between different actors (Farmer, Merchant, Customer) and core system functionalities including dashboard viewing, crop details access, region filtering, and chatbot assistance}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Level-0 DFD Context Diagram placeholder]}\vspace{2cm}}}
\caption{Level-0 Data Flow Diagram (Context Diagram) showing the AgroVision system as a central entity interacting with users and external data sources}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Level-1 DFD placeholder]}\vspace{2cm}}}
\caption{Level-1 Data Flow Diagram showing detailed processes including Dashboard Data Fetching, Crop Details Display, Price Prediction Generation, and Chatbot Response handling with their respective data flows}
\label{fig:dfd_level1}
\end{figure}

\textbf{Processes:}
\begin{enumerate}
    \item Fetch Dashboard Data
    \item Display Crop Details
    \item Generate Prediction
    \item Provide Chatbot Response
\end{enumerate}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[System architecture flowchart placeholder]}\vspace{2cm}}}
\caption{System architecture flowchart illustrating the complete data flow from user request through frontend components, backend processing, database queries, and external API calls to final response delivery}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Crop details flowchart placeholder]}\vspace{2cm}}}
\caption{Flowchart for viewing crop details showing the sequential process from user crop selection through data retrieval, chart rendering, prediction display, and news integration}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[AI Chatbot flowchart placeholder]}\vspace{2cm}}}
\caption{Flowchart for AI chatbot interaction depicting the process flow from user query submission through context extraction, LLM API communication, and response generation}
...
\begin{itemize}
    \item Fetches crop list with current prices
    \item Displays cards with sparkline charts
    \item Allows role-based filtering
\end{itemize}
\textbf{Internal Components:}
\begin{itemize}
    \item CropCard Component
    \item PriceService API
    \item RoleSelector Component
\end{itemize}

\subsection{Crop Detail Module}
\begin{itemize}
    \item Fetches crop-specific data
    \item Renders charts
    \item Displays prediction panel
\end{itemize}
\textbf{Internal Components:}
\begin{itemize}
    \item PriceChart Component
    \item PredictionPanel Component
    \item FactorsList Component
    \item NewsFeed Component
\end{itemize}
...
\begin{itemize}
    \item Floating button + modal
    \item Context-aware
    \item Communicates with LLM API
\end{itemize}
\textbf{Internal Components:}
\begin{itemize}
    \item ChatWindow
...
\textbf{Key elements:}
\begin{itemize}
    \item Clean grid layout
    \item Crop cards with:
    \begin{itemize}
        \item Images
        \item Price
        \item 24h movement indicator
        \item Mini chart
    \end{itemize}
\end{itemize}
...
\textbf{Sections:}
\begin{itemize}
    \item Header with price \& change
    \item Large interactive chart
    \item Region \& role filters
    \item AI prediction panel
    \item Influencing factors grid
    \item News section
\end{itemize}
...
\begin{itemize}
    ...
    \item Slide-up modal on mobile
    \item Summarize and explain options
\end{itemize}
...
\begin{itemize}
    ...
    \item Use cases
    \item Flow diagrams
    \item Module-level design
    \item Database schema
    \item User interface layouts
\end{itemize}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Implementation overview diagram placeholder]}\vspace{2cm}}}
\caption{Implementation overview diagram showing the technology stack including React JS frontend, Node.js backend, PostgreSQL database, and integration of ML models and external APIs}
...
\begin{itemize}
    ...
    \item Recharts/Chart.js for graphs
    \item Axios or Fetch API for backend communication
\end{itemize}

\subsection{Backend}
\begin{itemize}
    \item Node.js + Express (or Python FastAPI---based on team preference)
    \item REST APIs
    \item Weather, News, and Chatbot API integration
    \item ML-based prediction engine
\end{itemize}

\subsection{Database}
\begin{itemize}
    \item PostgreSQL / Supabase
    \item Tables for crops, price history, factors, news
\end{itemize}

\subsection{AI/ML}
\begin{itemize}
    \item Simple regression + factor-based adjustment predictor
    \item LLM (Claude/OpenAI-like) for chatbot interactions
\end{itemize}

\subsection{Deployment}
\begin{itemize}
    \item Hosted on a cloud platform such as Vercel/Netlify for frontend
    \item Render/AWS/Heroku for backend
    \item Supabase/Firebase for database
\end{itemize}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Frontend folder structure placeholder]}\vspace{2cm}}}
\caption{Frontend folder structure showing the organization of React components, services, utilities, and assets in the project directory}
...
\textbf{Functionalities Implemented:}
\begin{itemize}
    \item Fetch current price for each crop from backend
    \item Display:
    \begin{itemize}
        \item Image
        \item Name
        \item Current price
        \item 24h price movement (green/red badge)
        \item Mini trend graph
    \end{itemize}
\end{itemize}

\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Dashboard code snippet placeholder]}\vspace{2cm}}}
\caption{Code snippet demonstrating the dashboard implementation for fetching and displaying crop data with price information and trend indicators}
\label{fig:dashboard_code}
\end{figure}
...
\begin{itemize}
    \item Full-width interactive historical chart
    \item Dropdowns for:
    \begin{itemize}
        \item Region
        \item Time frame
        \item Role (Farmer/Merchant/Customer)
    \end{itemize}
    \item Prediction panel
    \item Influencing factors grid
    \item News articles feed
    \item Chatbot anchored to the page
\end{itemize}

\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Chart implementation code placeholder]}\vspace{2cm}}}
\caption{Code implementation example of the interactive historical price chart using Recharts library with dynamic data rendering}
\label{fig:chart_code}
\end{figure}
...
This panel displays:
\begin{itemize}
    \item AI predicted price
    \item Confidence indicator
    \item Explanation text
\end{itemize}

\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Prediction panel code placeholder]}\vspace{2cm}}}
\caption{Backend API implementation code for the prediction panel showing the endpoint structure for fetching AI-generated price predictions}
\label{fig:prediction_code}
\end{figure}
...
\begin{itemize}
    \item A floating button
    \item A modal window that opens when clicked
    \item A message input + AI response display
\end{itemize}

\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Chatbot code placeholder]}\vspace{2cm}}}
\caption{Chatbot implementation code demonstrating the message handling logic and API integration for conversational AI responses}
\label{fig:chatbot_code}
\end{figure}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Mobile app screenshot placeholder]}\vspace{2cm}}}
\caption{Screenshot of the AgroVision mobile application interface showing the crop dashboard with price cards and navigation elements}
\label{fig:mobile_screenshot}
\end{figure}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Backend folder structure placeholder]}\vspace{2cm}}}
\caption{Backend folder structure displaying the organization of routes, controllers, services, models, and configuration files}
...
\subsection{Prediction Algorithm (Simplified)}

\begin{enumerate}
    \item Fetch last 30 days price data
    \item Apply linear regression to estimate next period
    \item Apply seasonal multiplier
    \item Add/subtract factor impacts
    \item Generate confidence score
    \item Output predicted price
\end{enumerate}
...
Database created using Supabase/PostgreSQL.
...
\begin{itemize}
    \item Deployed on Vercel / Netlify
    \item Build is optimized using \texttt{npm run build}
\end{itemize}
...
\begin{itemize}
    \item API keys (News, Weather, Chatbot)
    \item Database connection URL
\end{itemize}
...
\begin{itemize}
    \item Unit tests for prediction logic
    \item API testing using Postman
    \item Manual UI testing on mobile and desktop
    \item Integration testing across frontend-backend
    \item Chatbot context testing
\end{itemize}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Unit testing interface placeholder]}\vspace{2cm}}}
\caption{Unit testing interface showing test cases for crop data retrieval, chart rendering, and prediction logic validation}
...
\begin{itemize}
    \item Some price graphs failed to load on mobile browsers $\rightarrow$ fixed by optimizing rendering logic
    \item Chatbot sometimes repeated previous responses $\rightarrow$ fixed by refreshing context data
    \item API response delays during prediction $\rightarrow$ optimized backend query structure
    \item UI elements overlapping on smaller screens $\rightarrow$ corrected with responsive styling
\end{itemize}
...
\textbf{Outcome:}
\begin{itemize}
    \item No layout breakage
    \item Buttons and charts scaled correctly
    \item Chatbot and filters worked across all devices
    \item Load times remained within acceptable limits
\end{itemize}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}\textit{[Cross-platform testing results placeholder]}\vspace{2cm}}}
\caption{Cross-platform testing results displaying AgroVision interface across different devices including mobile phones, tablets, and desktop browsers demonstrating responsive design implementation}
...
\begin{itemize}
    \item \textbf{User-Centric Design:} Early emphasis on understanding user needs across different stakeholder groups proved essential for creating a relevant and usable product.
    \item \textbf{Iterative Development:} Regular testing and feedback loops enabled continuous improvement and early detection of usability issues.
    \item \textbf{Balancing Complexity and Usability:} The challenge of presenting sophisticated analytics in an accessible manner required careful design decisions that prioritized clarity over feature richness.
    \item \textbf{Data Quality Importance:} The accuracy and reliability of predictions depend fundamentally on the quality of input data, highlighting the importance of robust data pipelines in production systems.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Frontend:} React JS with Tailwind CSS for responsive, intuitive user interfaces
    \item \textbf{Backend:} Node.js with Express for API services and business logic
    \item \textbf{Database:} PostgreSQL/Supabase for data storage and retrieval
    \item \textbf{AI/ML:} Regression-based prediction models and LLM integration for chatbot
\end{itemize}

Key features implemented include:
\begin{itemize}
    \item Interactive dashboard with crop price cards and trend indicators
    \item Detailed crop pages with historical charts and regional filtering
    \item AI-based price prediction with confidence scoring
    \item Influencing factors display (weather, supply-demand, seasonal patterns)
    \item News integration for market context
    \item Context-aware chatbot for user assistance
    \item Role-based views for farmers, merchants, and consumers
\end{itemize}
